...
**運用手順**: GAS の `readme.gs`（貼り替え・テスト・トラブル対応）。チームは git を見られないので、
運用に必要なことは GAS 側だけで完結させている。**この文書と重複させないこと。**
...
| やること | 読む節 | 目安 |
|---|---|---|
| **毎日の運用**<br>Slack を見る / 台帳を照合する | **この文書は不要。**GAS の `readme.gs` を読む | 15 分 |
...
| やること | 見るセクション | 行数 |
|---|---|---:|
| 機器・統制語彙を変える | `設定`（先頭） | 538 |
| 判定基準を変える | `通知判定` / `OS 該当・ベンダー別判定` | 969 |
| 台帳の列を変える | `設定` の `NW_LEDGER_COLS` と `台帳への記録` | 1,183 |
| Slack の見た目を変える | `Slack 通知` | 299 |
...
| **§2** スプレッドシート 5 枚 | 各シートの列と、その並び順の意味 |
...
| **Cisco の `vulnerabilities[].title` は脆弱性名** | 複数CVEのアドバイザリ11件中8件で CVE ごとに違う（例: ClamAV は ZIP / PDF / Mach-O と別々）。**未対応。現在は document.title を全行にコピーしている**（未着手。§6 にも載せていない） |
...
8. **AI へ渡しているもの**: アドバイザリの公開情報（タイトル・説明文・CVSS・影響製品）と、
   ...
   何を渡しているかは把握しておくこと（§6「10 月の新運用」に未判断として残っている）
...
### 4.15 「該当行が 0 件」と「取得できなかった」を区別する
...
対処は資産シートを正しく保つ運用のほう（§6「10 月の新運用」の資産棚卸しルール）。
