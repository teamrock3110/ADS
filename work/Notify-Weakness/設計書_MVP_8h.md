...
2. `migrateLedgerHeaders()` を実行（台帳 **13 列**）
...
## 7. 台帳・Slack の変更点（v6 → v7 → 表示修正）

### 台帳 13 列（2026-08 確定）

固定5列: `自社影響 | 製品 | CVE | CVSS | 最終更新日`  
判定表示: `公式推奨対応 | OS該当 | KEV | 影響機能 | 判定根拠`  
人の確認: `確認方法 | ユーザ影響`  
参照: `アドバイザリ`
...
|---|---|
| 自社影響 | **対応推奨 / 次回定期 / 要調査** |
| 公式推奨対応 | 判定に関わらず常に記入（次回定期でも空にしない） |
| KEV | **あり / なし** |
| 判定根拠 | 例: `OS=対象 \| KEV=なし \| 機能=Webフィルタ → 次回定期` |
| OS対象外 | **台帳に載せない**（処理済みのみ） |
| Cisco 判定 | KEV固定。表示列（影響機能・確認方法・ユーザ影響）は AI |
