...
1. **影響機能の 67% が「その他」** — 2026-09-03 に `countFeatures()` で実測。
   ...
   **いま並んでいる「影響調査」の 3 分の 2 は、懸念があるからではなくツールが
   分類できていないから。**確認する人はコマンドを打つ手がかりが無いまま行を渡されている。

   設計書v3 は月次サマリの保留理由に「実測で `不明` が約 3 割」を挙げていたが、
   統制語彙を入れた後の実測は対象行で 67%。悪化している。月次サマリはこの数字を
   毎月報告書に印字するので、消すか直すかの二択で、消すのは報告を嘘にする。

   まず `その他` の 6 行が何なのかを CVE・脆弱性名つきで一覧に出す。原因は
   「語彙に無い機能（表に足せば直る）」か「AI が分類に失敗（プロンプト側）」の 2 通り。

2. **10 月の新運用**（設計書v3 §2.3）— 対応基準の室長承認、資産棚卸しの運用ルール、
   判断記録を誰がいつ書くか。**ツールが先ではなく、基準が先。**
