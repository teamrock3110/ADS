...
### 4.11 月次サマリは突き合わせだけやる。文章は書かない・面を増やさない
...
| ファイル | 行数 | 関数 | 中身 |
|---|---|---|---|
| `fortinet_psirt_watcher_v7.gs` | 5,834 | 222 | 本体 |
| `fortinet_psirt_watcher_v7_tests.gs` | 615 | 23 | 動作確認用 |
...
1. **影響機能の 67% が「その他」** — 2026-09-03 に `countFeatures()` で実測。
   ...
   **2026-09-04 に解決した**（§4.9 / §4.10）。統制語彙を強制すると
   `normalizeCiscoFeature_` がタイトルから `IOS XE 基盤` を引き、露出が `always` になる。
   そのままだと `isSevereImpact_` が CWE 表現を読めず 4 件（CVSS 8.6）が「なし」に
   落ちたので、掌握系の CWE 語彙を足したうえで、**深刻度を判定できない行は
   「なし」ではなく「影響調査」に留める**分岐を入れた。

   結果（実データ 7 行）: 3 件が `対応検討` に上がり、4 件は `影響調査` のまま。
   根拠は「影響機能を特定できないため」から「影響の種類を特定できず深刻度を
   判定できないため」に変わった。**「なし」に落ちた行はゼロ。**

   **2026-09-06 に完了。**条件4の判定を CVSS ベクターへ移した（§4.9）ことで、
   残っていた 4 件も影響の種類が読めるようになった。Cisco が記述文に CWE 分類しか
   書いていなくても、ベクターには `C/I/A` が入っている。**他の情報源は不要だった**
   （NVD / CVE Program も当たったが、Cisco が CNA なので記述は同じ）。

   実データ 12 行の最終結果: 対応検討 7 / 影響調査 2 / なし 3。
   「なし」3 行はどれも条件2・条件3で落ちており、条件4まで来ていない。
