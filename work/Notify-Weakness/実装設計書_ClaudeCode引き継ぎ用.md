...
| ファイル | 行数 | 関数 | 中身 |
|---|---|---|---|
| `fortinet_psirt_watcher_v7.gs` | 5,782 | 222 | 本体 |
...
1. **影響機能の 67% が「その他」** — 2026-09-03 に `countFeatures()` で実測。
   ...
   **2026-09-04 に解決した**（§4.9 / §4.10）。統制語彙を強制すると
   ...
   残っているのは、4 件の「影響の種類を特定できない」をどう埋めるか。
   Cisco が CWE 分類しか公表していない以上、ツール側で埋める材料は無い。
   人が判断して判断記録に残す運用（§2.5）で受けるのが筋。
