...
| 情報の種別 | 表示方法 |
|-----------|---------|
| Tier 1/2 の公式ドキュメントから引用 | `source-link` クラスでインラインリンク |
| 公式情報が見つからない推測 | `speculation-tag` クラスで「※ 推測」と明示 |
...
<p class="step-body">
  ...
  <a href="https://support.claude.com/en/articles/8606394" target="_blank" class="source-link" style="font-size:0.75rem">
    ↗ Claude コンテキスト公式ヘルプ
  </a>
</p>
...
<!-- 良い例: 意外な制約の直後に出典 -->
<p class="step-body">
  Cowork は <strong>隔離された仮想マシン</strong> 上で動作する。
  <a href="https://www.anthropic.com/product/claude-cowork" target="_blank" class="source-link" style="font-size:0.75rem">
    ↗ Claude Cowork 公式
  </a>
</p>
...
```html
<!-- ステップ末尾に "出典:" をまとめる（同じステップ内の複数主張に対して）-->
<div class="step-body" style="margin-top:8px; display:flex; flex-direction:column; gap:4px;">
  <a href="..." target="_blank" class="source-link" style="font-size:0.75rem">↗ 出典名 A</a>
  <a href="..." target="_blank" class="source-link" style="font-size:0.75rem">↗ 出典名 B</a>
</div>
```
...
```html
<span class="speculation-tag">※ 推測（公式情報未確認）</span>
<!-- または -->
<span class="speculation-tag">公式情報なし</span>
```
...
- Lucide Icons（`<i data-lucide="...">`）を使う → 禁止（テキスト記号 `↗` を使う）
